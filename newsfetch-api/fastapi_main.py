...
from core import db_models, api_schemas, crud
...
async def create_article(article: api_schemas.Article, db: Session = Depends(get_db)):
...
async def update_article(article: api_schemas.Article, db: Session = Depends(get_db)):
    db_article = crud.get_article_by_url(db, url=article.url)
    if not db_article:
...
async def delete_article_by_url(url: str, db: Session = Depends(get_db)):
    try:
...
async def parse_article(parse_article_input: api_schemas.ParseArticleInput):
    newsplease_adapter = NewsPleaseHtmlAdapter(html=parse_article_input.html, url=parse_article_input.url)
    return api_schemas.Article(**newsplease_adapter.get_news())
