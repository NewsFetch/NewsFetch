...
class SpacyLanguageDetectionEnricher(SpacyEnricher):
    def __init__(self, model_name: str):
        super().__init__(model_name)

    def enrich(self, text: str) -> dict:
        logging.info("SpacyLanguageDetectionEnricher: detecting language...")
        logging.debug(f"SpacyLanguageDetectionEnricher: text is {text}...")

        doc = self.nlp(text, disable=["tok2vec", "tagger", "attribute_ruler", "lemmatizer"])
        detected_lang = doc._.language
        return {"detected_language": detected_lang["language"]}
...
if __name__ == "__main__":
    ...
    # spacy_language_detection_enricher = SpacyLanguageDetectionEnricher(config.SPACY_NER_EN_CORE_WEB_MD)
    # detected_lang = spacy_language_detection_enricher.enrich(content)
    # print(detected_lang)
