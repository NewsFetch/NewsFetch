...
class HuggingFaceSummarizationEnricher(HuggingFaceEnricher):
    def __init__(self, model_name: str):
        super().__init__(model_name)
        self.summarization_pipeline = pipeline(config.HUGGINGFACE_SUMMARIZATION_PIPELINE_NAME, model=self.model_name)
...
if __name__ == '__main__':
    ...
    summarization_enricher = HuggingFaceSummarizationEnricher(
        config.HUGGINGFACE_SUMMARIZATION_SSHLEIFER_DISTILBART_CNN_6_6)
    results = summarization_enricher.enrich(content)
    print(results)

    summarization_enricher = HuggingFaceSummarizationEnricher(config.HUGGINGFACE_SUMMARIZATION_GOOGLE_PEGASUS_XSUM)
    results = summarization_enricher.enrich(content)
    print(results)
